app.controller("choXacNhanCustomerController", function ($scope, $http) {
     ...
     $http.get("http://localhost:8080/customer/donHang/choXacNhan/" + decodedToken.email, { headers })
     ...
     $scope.customerhuyDon = function (hoaDon) {
          Swal.fire({
               ...
               if (result.isConfirmed) {
                    ...
                    $http.post("http://localhost:8080/customer/donHang/huyDonCustomer/", data, { headers })
                         .then(function (response) {
                              Swal.fire({
                                   icon: "success",
