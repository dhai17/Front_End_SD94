app.controller('DaThanhToanController', function ($scope, $http) {
    ...
    $scope.$watch('search', function (newVal) {
        if (newVal) {
            $http.get('http://localhost:8080/hoaDon/taiQuay/daThanhToan/timKiem=' + newVal, { headers }).then(function (response) {
                const pending = response.data;
    ...
    $scope.searchDateBill = function (searchDate) {
        ...
        $http.get('http://localhost:8080/hoaDon/taiQuay/daThanhToan/timKiemNgay=' + formattedDate, { headers }).then(function (response) {
            const pending = response.data;
