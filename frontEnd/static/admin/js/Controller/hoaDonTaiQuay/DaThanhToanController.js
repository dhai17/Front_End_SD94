...
app.controller("DaThanhToanController", function ($scope, $http) {
    ...
    $scope.$watch('search', function (newVal) {
        if (newVal) {
            $http.get("http://localhost:8080/hoaDon/taiQuay/daThanhToan/timKiem=" + newVal, { headers })
    ...
    $scope.searchDateBill = function (searchDate) {
        ...
        $http.get("http://localhost:8080/hoaDon/taiQuay/daThanhToan/timKiemNgay=" + formattedDate, { headers })
            .then(function (response) {
                const pending = response.data;
...
app.controller("HDCT_DaThanhToanController", function ($scope, $routeParams, $http) {
    ...
    $scope.loadData = function () {
        $http.get("http://localhost:8080/hoaDon/chiTietHoaDon/taiQuay/id=" + id, { headers })
        .then(function (response) {
