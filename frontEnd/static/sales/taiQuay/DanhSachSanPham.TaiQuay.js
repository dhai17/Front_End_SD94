app.controller("danhSachSanPhamTaiQuayController", function ($scope, $http) {
  ...
  $scope.$watch("searchTerm", function (newVal) {
    if (newVal) {
      $http
        .get(
          "http://localhost:8080/customer/sanPham/timKiemTheoTen/" + newVal,
          { headers }
        )
        .then(function (response) {
          $scope.sanPham = response.data;
  ...
  $scope.searchAll = function (searchTerm) {
    $http
      .get(
        "http://localhost:8080/customer/sanPham/timKiemTheoTen/" + searchTerm,
        { headers }
      )
      .then(function (response) {
        $scope.sanPham = response.data;
...
app.controller(
  "ChiTietSanPhamTaiQuayController",
